...
class Surveys::CreateCurrentWeekSurveyTest < ActiveSupport::TestCase
  ...
  test "uses the next Thursday when today is Thursday" do
    travel_to Time.zone.local(2026, 6, 11, 12, 0) do
      survey = Surveys::CreateCurrentWeekSurvey.call

      assert_equal "2026-06-18", survey.title
      assert_equal Time.zone.local(2026, 6, 18, 0, 0), survey.start_at
      assert_equal Time.zone.local(2026, 6, 20, 0, 0), survey.end_at
    end
  end
